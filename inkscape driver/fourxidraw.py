...
class FourxiDrawClass(inkex.Effect):
	...
	def __init__(self):
		...
		self.PenDownSpeed = fourxidraw_conf.PenDownSpeed * fourxidraw_conf.SpeedScale # Default speed when pen is down		
		self.PenUpSpeed = 0.75 * fourxidraw_conf.SpeedScale # Default speed when pen is down	
		...
		if (self.options.mode == "manual"):
			if (self.options.manualType == "none"):
				...
				for node in self.svg.xpath('//svg:eggbot', namespaces=inkex.NSS):
					self.svg.remove(node)
				inkex.errormsg(gettext.gettext("I've removed all 4xiDraw data from this SVG file. Have a great day!"))
				return	
